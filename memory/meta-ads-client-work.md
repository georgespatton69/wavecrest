...
**Meta advertiser verification (rolling out across ALL accounts, 2026-09-08):** Ads Manager shows an "Act now — Verification may be required soon. Complete it now to avoid possible future ad disruptions" banner, then a **"Verify yourself or an organization"** dialog with two routes: **(1) Verify an organization** (Meta-"Recommended") = match entity to an official record + prove the link via a **business-domain email**/direct connection → public Ad Library disclosure shows the BRAND; more durable but more friction. **(2) Verify yourself** = government **ID + possible selfie** → fast, but the **personal name may show publicly** in Ad Library. **User's default = "Verify yourself" (ID)** — did ABH this way 2026-09-08 (see [[client-addiction-behavioral]]). ⚠️ **Do these ONE AT A TIME, let each settle** — rapid identity/verification steps across the many accounts on this single Robby Hoffman login have tripped a Meta **security lock** before. If skipped past the deadline, delivery pauses. Whatever route, entity/name details must match that account's business + LegitScript record char-for-char.
